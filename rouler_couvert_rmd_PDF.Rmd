...
Ces variables sont présentées dans le tableau suivant :
...
L’un des premiers problèmes que nous avons rencontrés est le nombre d’observations dans la base de données.
...
Pour pallier à ce problème, deux solutions se proposaient à nous :
...
Étant donné que nous disposons d’un nombre conséquent d’observations, nous avons alors choisi la deuxième option.
...
Ainsi, tous les clients sont récents.
...
1. **n_trees** : Ce paramètre détermine le nombre total d'arbres de décision à construire dans le modèle de Boosting. Une valeur plus élevée peut améliorer la performance du modèle mais aussi augmenter le risque d'overfitting et le temps de calcul.
...
Le modèle Boosting présente une accuracy de  `r Accuracy(matriceBOOST_t) |> round(2)` %, ce qui le place entre les modèles Decision Tree et Random Forest pour cet indicateur. Pour la sensibilité, elle est de `r Sensibilite(matriceBOOST_t) |> round(2)` %, ce qui représente une légère diminution par rapport à Random Forest (`r Sensibilite(matrice_rf_t) |> round(2)` %) et plus significative par rapport au Decision Tree (`r Sensibilite(matrice_dt_t) |> round(2)` %).Et sur la Precision, elle est de `r Precision(matriceBOOST_t) |> round(2)` %, légèrement inférieure à Random Forest (`r Precision(matrice_rf_t) |> round(2)` %) mais supérieure à Decision Tree (`r Precision(matrice_dt_t) |> round(2)` %).
